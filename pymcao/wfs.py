...
class WFS(object):
    ...
    def __init__(self, config):
        ...
        # Verbose mode, FFT mode, wavelength and telescope diameter
        self.verbose = config.verbose
        self.fft_mode = config.fft_mode
        self.wavelength = config.wavelength
        self.telescope_diameter = config.telescope_diameter
        self.noise_std_wfs = config.noise_std_wfs

        # Remove tip-tilt and subaperture geometry in the pupil plane
        self.remove_tiptilt = config.remove_tiptilt
        self.subapertures_geometry = config.geometry
        self.enhancement = config.enhancement

        # Number of subapertures in the horizontal direction in the pupil plane
        self.n_subapertures_horizontal = config.n_subapertures_horizontal

        # self.npix_pupil_subaperture = config.npix_pupil_subaperture
        self.gap_subaperture = config.gap_subaperture
        
        # Simulation pixel size
        self.simulation_pixel_size = config.simulation_pixel_size

        # WFS pixel size and size of each subaperture (image plane)
        self.wfs_pixel_size = config.wfs_pixel_size
        self.wfs_subaperture_arcsec = config.wfs_subaperture_arcsec
        
        # Number of pixels of each subaperture in the WFS in units of the pixel
        # size of the WFS
        self.npix_wfs_subaperture = int(self.wfs_subaperture_arcsec / self.wfs_pixel_size)
        
        # Now compute the number of pixels in the simulation pixel size
        # corresponding to npix_wfs_subaperture (enhance by a factor to allow
        # for apodization)
        self.npix_pupil = int(self.enhancement * self.wfs_pixel_size / self.simulation_pixel_size * self.npix_wfs_subaperture)
        
        # Compute number of pixels per subaperture in the pupil
        # Adapt enhancement factor to have an fit the number of subapertures in
        # the pupil
        if (self.subapertures_geometry == 'shifted'):
            self.npix_pupil_subaperture = int(np.ceil((self.npix_pupil - self.gap_subaperture * (self.n_subapertures_horizontal + 1)) / (self.n_subapertures_horizontal + 1)))
        ...
        # Compute the total number of pixels in the WFS image plane. This is
        # just to simulate the final appearance of the WFS but one could work
        # simply with the image of each subaperture
        if (self.subapertures_geometry == 'shifted'):
        ...
        self.pupil = util.aperture(npix=self.npix_pupil, cent_obs = config.central_obscuration / config.telescope_diameter, spider=0)
        ...
        if (self.verbose):
            ...
            self.logger.info(f"Simulation pixel size : {self.simulation_pixel_size} arcsec (WFS)")            
            self.logger.info(f"Pixel size in WFS : {self.wfs_pixel_size} arcsec (WFS)")
            self.logger.info(f"Number of pixels of subaperture in WFS : {self.npix_wfs_subaperture} (WFS)")
            self.logger.info(f"Size of subaperture in WFS : {self.wfs_subaperture_arcsec} arcsec (WFS)")
            self.logger.info(f"Enhacenment in WFS (recomputed for fitting subapertures): {self.enhancement} (WFS)")
            self.logger.info(f"Noise std in WFS: {self.noise_std_wfs} (WFS)")
            
            self.logger.info(f"Number of pixels per subaperture in pupil : {self.npix_pupil_subaperture} (PUPIL)")            
    ...
    def compute_subapertures(self, fill_fraction=1.0):
        ...
        if (self.subapertures_geometry == 'shifted'):
            bottom = int(-0.5 * self.npix_pupil_subaperture)
        ...
        for i in range(self.n_subapertures_horizontal+2):
            ...
            top = bottom + self.npix_pupil_subaperture
            ...
            for j in range(self.n_subapertures_horizontal+2):
                ...
                if (np.sum(mask2 * self.pupil) >= fill_fraction * self.npix_pupil_subaperture**2):
                    ...
                    mask_subaperture[r_subp <= 0.5*self.npix_pupil_subaperture] = 1
    ...
    def generate_wfs_images(self):
        ...
        # Initialize the image with the xy displacements
        self.ratio_image = np.zeros((self.npix_wfs, self.npix_wfs))
        ...
        im_final += np.random.normal(loc=0.0, scale=self.noise_std_wfs, size=im_final.shape)
    ...
    def measure_wfs_correlation(self):
        ...
        for i in range(self.n_subapertures_total):
            ...
            r_fft_noise = np.fft.fft2(im2) / t1
            ratio = np.fft.fftshift(np.fft.ifft2(r_fft_noise * mask))
            ratio = np.sqrt(ratio * np.conj(ratio)).real
            ...
            self.ratio_image[bottom:top, left:right] = ratio
...
if (__name__ == '__main__'):
    np.random.seed(123)
    wfs = WFS(telescope_diameter = 100.0, n_subapertures_horizontal = 9, npix_pupil_subaperture = 45, gap_subaperture = 4, wfs_pixel_size=0.2, verbose=True, fft_mode='pyfftw', \
    ...
    wfs.generate_wfs_images(noise_std=1e2)
